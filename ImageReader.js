      class ImageReader {
        ...
        async downloadMedia(media_obj){
          try {
            ...
            if( !headers.type.includes('image') ) throw { message: 'Header content-type not an image.', headers: headers, media_obj: media_obj };
            ...
            if( ratio < min_ratio || ratio > max_ratio || image.naturalWidth < min_width || image.naturalHeight < min_height ){
              throw { message: 'Image dimenions out of range.', ratio: ratio, width: image.naturalWidth, height: image.naturalHeight, media_obj: media_obj };
            }
            if(animated){
        ...
        async get_embed_objs(string){
          var embed_sources = [
            ...
            { brand: 'reddit-media', height: 512, template: 'https://www.redditmedia.com/mediaembed/{id}?responsive=true', regexp: /https:\/\/www\.redditmedia\.com\/mediaembed\/(\w+)/gs },
            //{ brand: 'reddit-embed', height: 512, template: '{id}', regexp: /(https:\/\/www\.reddit\.com\/r\/(?:[\d|a-zA-Z|\-|_]+)\/comments\/(?:[\d|a-zA-Z|\-|_]+)\/(?:[\d|a-zA-Z|\-|_]+))\//gs },
          ...
          var embeds = await Promise.all( embed_sources.map(async function(source){ // map sources
            var embed_matches = string.match(source.regexp);
            if( embed_matches == null ) return [];
        ...
        async get_media_objs(string){
          ...
          //console.log(string)
          var selector = 'source,img,a,iframe,media\\:content,media\\:thumbnail'; // meta[property*="image"],[style*="background"]
          ...
          return await Array.from( await Promise.all( Array.from(nodes).map(async function(node){
            return await Array.from( await Promise.all( Array.from(node.attributes).map(async function(attribute){
              ...
              if(attribute.name != 'href' && i < 0){ // get image src values that don't have extensions and set to jpeg
         ...
         *  @param {array.<media>} [to_merge] - An array of media URLs to merge with
         *  @returns {(array<media>|boolean)} - An array of media objects
         */
        async getMediaInString(string,to_merge){
          try {
            ...
            return merged.filter(function(media,i){ // get unique values
              return merged.findIndex(function(unique_media) {
                return unique_media.url == media.url;
              }) == i;
            });
          } catch(e) {
            console.log(e);
        ...
        async parseFeed(string){
          try {
            ...
            return await Promise.all( Array.from(item_nodes).map(async (node) => {
              ...
              let media_content_objs = await this.getMediaInString(node.textContent);
              return {
                ...
                media_objs: await this.getMediaInString(node.innerHTML.replaceAll(/<!\[CDATA\[|\]\]>/g,''), media_content_objs),
              }
            }));
          } catch(e) {
            console.log(e);
